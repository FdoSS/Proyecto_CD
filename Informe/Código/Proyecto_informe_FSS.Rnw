...
media_ppltrst <- mean(datos$ppltrst)
media_pplfair <- mean(datos$pplfair)
media_pplhlp <- mean(datos$pplhlp)

@


El numero total de participantes en el estudio es \emph{\Sexpr{nrow(datos)}}.
